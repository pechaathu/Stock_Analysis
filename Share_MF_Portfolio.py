...
if st.session_state.page == "Stock Portfolio":
    ...
    with coltree:
        dftree = df.copy()
        stockslist = dftree['Stock Name'].unique().tolist()
        stockslist = [i for i in stockslist if dftree[dftree['Stock Name']==i]['Current Holdings'].min()>0]
        dftree = dftree[dftree['Stock Name'].isin(stockslist)].copy()
        fig_tree = px.treemap(dftree, path=["Sector"], values="Total Amount", color="Sector", color_discrete_sequence=px.colors.qualitative.Set1)
